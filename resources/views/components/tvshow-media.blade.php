<div class="tvshow-list ml-0 sm:ml-0 md:ml-24 mt-12 glide__backdrops relative">
    <div class="tvshow-heading"><h4 class="tracking-widest font-medium text-xl">MEDIA ({{ count($tvshow['backdrops']) }})</h4></div>
    @if(count($tvshow['backdrops']) > 0)
    <div class="flex glide__track" data-glide-el="track" x-data="{ isOpen: false, image: '' }">
        <ul class="glide__slides scrolling-auto overflow-visible">
            @foreach ($tvshow['backdrops'] as $backdrop)
