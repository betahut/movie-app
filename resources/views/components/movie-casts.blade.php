                ...
                <li class="mt-8 {{ (!$loop->last) ? 'mr-2' : '' }} glide__slide w-auto">
                    <a href="#" class="flex justify-center items-center transition ease-in-out duration-150 ">
                        @if(trim($cast['profile_path']) != '')
                            <img src="{{ 'https://image.tmdb.org/t/p/w185/'.$cast['profile_path'] }}" alt="cast" class="w-32 h-32 object-contain rounded-full hover:opacity-75 transition ease-in-out duration-150 border border-gray-300" />
                        @else
