                ...
                <li class="mt-8 {{ (!$loop->last) ? 'mr-2' : '' }} glide__slide w-auto">
                    <div class="flex justify-center items-center transition ease-in-out duration-150 ">
                        @if(trim($backdrop['file_path']) != '')
                            <img src="{{ 'https://image.tmdb.org/t/p/w780/'.$backdrop['file_path'] }}" alt="cast" class="object-contain hover:opacity-75 transition ease-in-out duration-150" />
                        @endif
                    </div>
                </li>
            @endforeach
                ...
                <li class="mt-8 {{ (!$loop->last) ? 'mr-2' : '' }} glide__slide w-auto">
                    <div class="flex justify-center items-center transition ease-in-out duration-150">
                        @if(trim($poster['file_path']) != '')
                            <img src="{{ 'https://image.tmdb.org/t/p/w780/'.$poster['file_path'] }}" alt="cast" class="object-contain hover:opacity-75 transition ease-in-out duration-150" />
                        @endif
                    </div>
                </li>
            @endforeach --}}
