    ...
    <div class="content">
        ...
        <div class="p-8 z-10 relative z-10">
            <div class="flex flex-col lg:justify-between items-start mt-8 ml-0 sm:ml-0 md:ml-24">
                ...
                <div class="movie-div grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    @foreach($popularMovies as $movie)
                    <div class="movie mt-8 relative w-100">
                        <div class="absolute z-10 transform -rotate-90 inset-auto opacity-75 hover:opacity-100 transition ease-in-out duration-150 w-40" style="bottom: 105px; left: -85px;">
                            <span class="text-md text-6xl font-black truncate text-red-700">{{$movie['vote_average']}}</span>
                        </div>
                        <div class="relative z-0">
                            <a href="{{ route('movies.show', $movie['id']) }}">
                                <img src="{{$movie['poster_path']}}" alt="" class="hover:opacity-75 transition ease-in-out duration-150 rounded-lg" style="height:278px; width: 178px;">
                            </a>
                            <div class="mt-2">
                                <a href="{{ route('movies.show', $movie['id']) }}" class="text-md truncate hover:text-gray-300" title="{{$movie['title']}}">
                                    <div class="truncate">{{$movie['title']}}</div>
                                </a>
                                <div class="text-sm truncate text-gray-400" title="{{$movie['genres']}}">{{$movie['genres']}}</div>
                            </div>
                        </div>
                    </div>
                    @endforeach
                ...
                {{-- <div class="flex justify-between mt-16 w-full">
                    @if($previous)
                        <a href="{{ route('movie.page', $previous) }}" class="relative">
                    ...
                    @endif
                    @if($next)
                        <a href="{{ route('movie.page', $next) }}" class="relative">
